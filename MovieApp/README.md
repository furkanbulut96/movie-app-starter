...
1*Tailwind Kuruldu
2*Registerin Form yapısı Tailwind ile yapıldı
3*AppRoutera sarmallaması yapıldı
4* Navbar düzenlendi
5*FireBase geçiş
6*Auth için Contextimiz yapıyoruz
7*FireBase kurulumlarına dönüyoruz
8*Movie Context yapıp context sayesinde Maine verileri yolladık
9* Mainden Main Carda yolladık gerekli işlemler yapıldı
10*Switch Comp (darkMod(Toogle)) ve Video Section ile Youtuba